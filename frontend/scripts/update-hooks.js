...
const jsFile = resolve(root, "frappe_copilot/public/frontend/dist/js/frappe_copilot.js");
const cssFile = resolve(root, "frappe_copilot/public/css/copilot.css");
const hooksFile = resolve(root, "frappe_copilot/hooks.py");
...
hooks = hooks.replace(
  /(frappe_copilot\.js)(?:\?v=[a-f0-9]+)?/,
  `$1?v=${jsHash}`,
);

// Replace or append ?v=<hash> on the CSS line
hooks = hooks.replace(
  /(copilot\.css)(?:\?v=[a-f0-9]+)?/,
  `$1?v=${cssHash}`,
);
